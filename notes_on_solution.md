...
Run the following to seed the db with some test events and print to console the output of the rolled-up table.
...
A few notes to navigate my changes. The current state contains two changes: 
  + A new migration at `lana/app/migrations/20250617085344_user_rollup.sql` that deploys all the changes I've made to the admin pg server.
  + A new `bats` test to check that the rollup leaves the target table in the desired state after seeding some events.

About the migration:
* We create a simple table `users`. Fields are both metadata and actual `user` attributes. Hopefully the table feels self-descriptive.
...
* Functions `fn_project_user_initialized`,                       `fn_project_user_authentication_id_updated`,                       `fn_project_user_role_granted`,  `fn_project_user_role_revoked` implement the business logic. They are dead simple, the most straightforward explanation is simply to take a look at them.
...
* Currently, should a failure happen, the whole transaction, including the insert of the event, won't commit. That's *good*. But there's no trigger specific logging, so if the trigger/rollup functions cause an exception, we might be left wondering why the event couldn't be persisted. Again, we would need to discuss what would make sense here because I'm not fully familiar with what is the server already logging.
* Defining entity snapshot tables, triggers, what events must be reacted to, and how they impact the entity, is terribly manual. It's work, plus someone must be at the wheel paying attention to ensure it's in sync with what's defined in `core`.
...
* Regarding generating (instead of handcrafting) all this house of cards of SQL for each entity/event stream, I think there are differnent approaches along the complexity/usefulness. From less to more:
  + Implement consistency checkers:
    ...
    - Make some code to parse the entity table, trigger and functions and cross check consistency with rust definitions or json schemas derived from them.
    - Doesn't help you build, but it helps noticing that you're screwing up when handcrafting.
    - Although I find kind of pointless if `e2e` testing of the rollup behaviour is thorough enough, and that's somethingt we should do regardless.
  + Generate SQL boilerplate programmatically:
    - Create entity snapshot DDL from code definitions.
      - Side note: It's funny how there is no single clear definition of what are the attributes of `core_user`. User events tell you one story, but the code at `core/access/src/user/repo.rs` tells you another (e.g events indicate there's a `RoleId`, but the columns in `core/access/src/user/repo.rs` don't mention any role related field).
    - Create trigger and mapping functions boilerplate from `core/access/src/user/entity.rs` `UserEvent`.
    - Create event rollup functions boilerplate, BUT leave it up to the developer to implement the effects of the event on the rollup table, since this is the hardest bit to define strictly outside of code.
  + Generate SQL boilerplate AND contents programmatically:
    ...
    - The final (and most challenging) cherry on top bullet would be to generate the rollup functions logic. Lots of changes would be required for this, and I'm wondering if the complexity is worth it. At most, I see some entities and events that follow very restricted and simple patterns being good candidates for this level of complexity. Other's might need to be implemented completely manually, or our code generation framework would need to implement escape hatches to allow for "free form" logic to be added. Happy to discuss on the topic.
